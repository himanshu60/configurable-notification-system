...
The whole system deploys as **one service**. The Express process serves the API *and* the built
Angular client, so there is a single URL, no CORS, no rewrite rules, and no API URL compiled into
the bundle — `environment.production.ts` uses the relative path `/api/v1`.

A [`render.yaml`](render.yaml) blueprint defines it: **New → Blueprint → point at this repo**.
...
| Field | Value |
|---|---|
| Root Directory | *leave empty* — the workspace root is where the lockfile lives |
| Build Command | `npm ci --include=dev && npm run build -w @cns/shared && npm run build -w @cns/api && npm run build -w @cns/web` |
| Start Command | `node apps/api/dist/server.js` |
| Health Check Path | `/health/ready` |

`--include=dev` matters: with `NODE_ENV=production` npm skips devDependencies, and the build needs
...
| Key | Value |
|---|---|
| `MONGODB_URI` | a MongoDB Atlas connection string (free M0 tier is enough) |
| `JWT_SECRET` | any 32+ character random string — Render can generate it |
| `SERVE_WEB` | `true` — serves the Angular build from this process |
| `NODE_ENV` | `production` |
| `WORKER_ENABLED` | `true` |

Everything else has a sensible default; see [`.env.example`](.env.example).

In Atlas, allow access from anywhere (`0.0.0.0/0`) — Render's free tier has no static egress IPs.
...
```bash
MONGODB_URI="mongodb+srv://USER:PASS@cluster.mongodb.net/cns" npm run seed
```

Or just register a fresh account through the UI — the seed only exists to give a reviewer
instant data.

### Splitting it into two services

Serving static files from Node is fine at this scale but couples the client's availability to the
API's. To separate them: set `SERVE_WEB=false`, add a Render **Static Site** publishing
`apps/web/dist/web/browser`, and give it two rewrites in order — `/api/*` to the API service,
then `/*` to `/index.html`. Nothing in the application code changes.

### Free-tier behaviour

Free services sleep after ~15 minutes idle, so the first request wakes them and the worker stops
while asleep. Queued deliveries are not lost — they are rows with `status: PENDING`, and the
worker drains them on the next wake. That is the outbox design paying off, but it does mean
delivery latency is unbounded while the service is asleep.
