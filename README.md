...
A [`render.yaml`](render.yaml) blueprint defines both services. From the Render dashboard:
**New → Blueprint → point it at this repo**.

| Service | Type | Serves |
|---|---|---|
| `cns-api` | Web service (Node) | Express API + the in-process delivery worker |
| `cns-web` | Static site | The Angular build |

**What you need before deploying**

1. **A MongoDB Atlas cluster** (the free M0 tier is enough). Copy its connection string and set
   `MONGODB_URI` on the `cns-api` service — it is marked `sync: false` in the blueprint precisely
   so the secret is never committed. Allow Render's egress IPs in Atlas, or `0.0.0.0/0` for a demo.
2. **Nothing else.** `JWT_SECRET` is generated by Render, and every other variable has a default in
   the blueprint.

**How the two services talk.** The static site rewrites `/api/*` to the API service, so the browser
only ever sees one origin. That means no CORS preflight and — importantly — no API URL compiled
into the bundle: `environment.production.ts` uses the relative path `/api/v1`, so the same artifact
would work in staging or production unchanged. Update the `destination` in `render.yaml` if you
rename the API service.

**Seeding the demo data** (optional): open a Shell on `cns-api` and run `npm run seed -w @cns/api`.

**Things to know about the free tier**

- Free services sleep after ~15 minutes idle, so the first request wakes them and the in-process
  worker stops while asleep. Queued deliveries are not lost — they are rows with
  `status: PENDING`, and the worker drains them on the next wake. That is the outbox design paying
  off, but it does mean delivery latency is unbounded while the service is asleep.
- For anything real, run the worker as its own always-on service: set `WORKER_ENABLED=false` on the
  API and add a second service with it `true`. The atomic claim already makes that safe.
